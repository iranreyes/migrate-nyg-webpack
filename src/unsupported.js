import matrix from './util/device-matrix.json';
import device from './util/detect/util-os';
import browser from './util/detect/util-browser.js';

var supported = false;
var version = parseFloat(browser.checkVersion());
var os = device.os().toLocaleLowerCase();
var osVersion = parseFloat(device.osVersion());
...
if (browser.checkBot()) {
  ...
  if (
    ...
    osVersion >= matrix.android &&
    version >= matrix.chrome
  )
    supported = true;
} else if (
  (browser.checkIE() && version >= matrix.ie) ||
